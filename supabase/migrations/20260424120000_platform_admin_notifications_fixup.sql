-- Fix-up: tidligere push errede halvvejs (table fandtes allerede) → garantér at funktionerne også er på plads.
-- Alle statements er idempotente: tabel/policies tjekker eksistens, funktioner bruger CREATE OR REPLACE.

create table if not exists public.platform_admin_seen (
...
drop policy if exists "Own seen row read" on public.platform_admin_seen;
drop policy if exists "Own seen row upsert" on public.platform_admin_seen;
drop policy if exists "Own seen row update" on public.platform_admin_seen;
