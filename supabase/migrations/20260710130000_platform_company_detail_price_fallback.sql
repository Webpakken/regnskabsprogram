-- Bilago: platform-detaljer viste tom Plan/Pris når et abonnement kun havde
-- stripe_price_id sat (billing_plan_id null — fx sat direkte af Stripe-webhook
-- før planen blev linket). Fald tilbage til at matche billing_plans på
-- stripe_price_id, så plannavn og pris altid vises.
  ...
  select jsonb_build_object(
    ...
    'subscription', (
      ...
      left join public.billing_plans bp
        on bp.id = s.billing_plan_id
        or (s.billing_plan_id is null and bp.stripe_price_id = s.stripe_price_id)
...
-- Bagudfyld manglende billing_plan_id ud fra stripe_price_id, så resten af
-- appen (og fremtidige queries) også kender planen.
update public.subscriptions s
set billing_plan_id = bp.id
from public.billing_plans bp
where s.billing_plan_id is null
  and s.stripe_price_id is not null
  and bp.stripe_price_id = s.stripe_price_id;
